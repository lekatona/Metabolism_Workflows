...
##   Adjust GLDAS data to a necessary time resolution
##   based on other data being used in the model.
## This code uses an inelegant function to convert
##   datetime of the data
########################################################

### Import packages
lapply(c("dplyr", "readr", "tibble", "lubridate",
         "dygraphs", "tidyr", "tidyselect", "zoo",
         "xts", "pracma", "reshape2", "Hmisc",
         "ggplot2", "grid", "gridExtra", "wesanderson",
         "data.table", "RColorBrewer", "plyr", "stringr",
         "chron", "streamMetabolizer"), require, character.only=T)


## Read in data
setwd("C:/Users/leonk/Dropbox/Blaszczak-Lab-Metabolism-Code-Working-Files")
...
## Convert DateTime. This function is set up for using GLDAS data from 1980
##   to 2022. Adjust this according to your data and needs. 
...
Create_DT <- function(x){
  ...
  ## Want 15 minute data
  #sampfreq = "15M"; use "5M" for 5 minute data
...
vis_spread <- function(x){
  ...
  dygraph(Rnew_xts)%>% dyAxis("y", valueRange= c(82500, 90000))  
}

vis_spread(GLDS.data_spread$Reno_GLDAS_Pressure.csv)
